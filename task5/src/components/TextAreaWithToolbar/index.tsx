...
import style from '@/ui/TextAreaWithToolbar/TextAreaWithToolbar.module.scss';
...
export default function TextAreaWithToolbar() {
  ...
  const [listMode, setListMode] = useState<'none' | 'bullet' | 'numbered'>(
    'none'
  ); // Режим списка
  ...
  const handleInput = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (textarea && listMode !== 'none') {
      ...
      if (
        ...
        const newValue =
          value.substring(0, lastNewlineIndex + 1) +
          prefix +
          value.substring(lastNewlineIndex + 1);
        setText(newValue);
      } else {
  ...
  const applyStyle = (style: string) => {
    ...
    if (style === 'bold') {
      newStyles.fontWeight =
        newStyles.fontWeight === 'bold' ? 'normal' : 'bold';
    } else if (style === 'italic') {
      newStyles.fontStyle =
        newStyles.fontStyle === 'italic' ? 'normal' : 'italic';
    } else if (style === 'code') {
      newStyles.fontFamily =
        newStyles.fontFamily === 'monospace' ? 'inherit' : 'monospace';
    }
  ...
  return (
    <div className={style['textarea-wrp']}>
      ...
      <div className={style['textarea-header']}>
        ...
        <button onClick={() => applyFormat('numbered')}>
          Нумерованный список
        </button>
      </div>
